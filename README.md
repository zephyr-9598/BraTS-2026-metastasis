...
```bash
nnUNetv2_plan_and_preprocess -d DATASET_ID -verigy_dataset_integrity
```
...
```bash
Use the nnUNetv2_train command with no change
```
...
nnUNetv2_train DATASET_ID configuration fold -tr nnUNEtReliabilityweights

# Train with aleatoric weighting if needed
```

### Inference

```bash
Use the inference command generated by nnUNet at inference.txt
```
...
├── src/
│   ├── nnUNet_epistemictrainer.py         # Purely epistemic estimation
│   ├── nnUNet_epistemic_spatial.py        # Spatial Epistemic estimation
│   ├── nnUNet_aleatoric.py                # Aleatoric estimation
│   └──  nnUNet_aleatoric2.py              # Beta version of new experiments
├── scripts/
│   ├── reliability_weights.py             # Reading Scalar reliability weights
│   └── reliability_weights_spatial.py     # Reading the reliability weights for the spatial case
└── README.md
```
