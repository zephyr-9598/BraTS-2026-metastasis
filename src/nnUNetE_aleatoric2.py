...
class AleatoricSegmentationLoss(torch.nn.Module):
    ...
    """
    def __init__(self, smooth=1e-5, variance_clamp=(-2, 2)):
        super().__init__()
        self.smooth = smooth
...
class nnUNetTrainerAleatoric(nnUNetTrainerDiceCELoss_noSmooth):
    ...
    def __init__(self, plans, configuration, fold, dataset_json, device=torch.device('cuda')):
        ...
        self.variance_clamp = (-2, 2)  # clamp to avoid extreme values
...
class nnUNetTrainerAleatoricFixedLog(nnUNetTrainerAleatoric):
    """Same as nnUNetTrainerAleatoric but with the correct log-variance regularizer."""
    def _build_loss(self):
        base_loss = AleatoricLossWrapper(
            AleatoricSegmentationLossFixedLog(variance_clamp=self.variance_clamp),
            self.label_manager.num_segmentation_heads
        )
        if self.enable_deep_supervision:
            scales = self._get_deep_supervision_scales()
...
class AleatoricSegmentationLossFixedLog(AleatoricSegmentationLoss):
    def forward(self, logits, logvar, target):
        logits = logits.float()
        logvar = logvar.float()
        logvar = torch.clamp(logvar, min=self.variance_clamp[0], max=self.variance_clamp[1])
        sigma_sq = torch.exp(logvar)
        tgt_squeezed = target.squeeze(1).long()

        ce_per_voxel = F.cross_entropy(logits, tgt_squeezed, reduction='none').unsqueeze(1)
        loss_ce = (ce_per_voxel / (2 * sigma_sq)).mean()

        loss_reg = 0.5 * logvar.mean()   # <-- the fix

        probs = torch.softmax(logits, dim=1)
        tgt_onehot = F.one_hot(tgt_squeezed, num_classes=logits.shape[1]).permute(0, 4, 1, 2, 3).float()
        weight_map = 1.0 / (2 * sigma_sq)
        dims = (0, 2, 3, 4)
        intersection = (weight_map * probs * tgt_onehot).sum(dims)
        cardinality = (weight_map * (probs + tgt_onehot)).sum(dims)
        dice_per_class = (2 * intersection + self.smooth) / (cardinality + self.smooth)
        loss_dice = 1.0 - dice_per_class[1:].mean()

        return loss_ce + loss_reg + loss_dice
    

class AleatoricSegmentationLossFreqWeighted(AleatoricSegmentationLoss):
    def __init__(self, class_frequencies: dict, smooth=1e-5, variance_clamp=(-2, 2)):
        super().__init__(smooth=smooth, variance_clamp=variance_clamp)
        # class_frequencies keyed by foreground label id (1..N), background not included
        sorted_labels = sorted(class_frequencies.keys())
        freqs = torch.tensor([class_frequencies[l] for l in sorted_labels], dtype=torch.float32)
        alpha_fg = (1.0 / freqs)
        alpha_fg = alpha_fg / alpha_fg.sum() * len(alpha_fg)   # normalize so mean weight ~1
        # background gets weight 1.0, prepended at index 0
        alpha = torch.cat([torch.tensor([1.0]), alpha_fg])
        self.register_buffer_alpha(alpha)

    def register_buffer_alpha(self, alpha):
        # not an nn.Module param we backprop through, just store for device movement
        self.alpha = alpha
    ...
    def forward(self, logits, logvar, target):
        ...
        ce_per_voxel = F.cross_entropy(logits, tgt_squeezed, weight=alpha, reduction='none').unsqueeze(1)
        loss_ce = (ce_per_voxel / (2 * sigma_sq)).mean()
        ...
        cardinality = (weight_map * (probs + tgt_onehot)).sum(dims)
        dice_per_class = (2 * intersection + self.smooth) / (cardinality + self.smooth)
        # apply alpha to fg classes only, same normalization used above
        loss_dice = 1.0 - (alpha[1:] * dice_per_class[1:]).sum() / alpha[1:].sum()

        return loss_ce + loss_reg + loss_dice


class nnUNetTrainerAleatoricFreqWeighted(nnUNetTrainerAleatoric):
    class_frequencies = None  # set on subclass or instance before training starts

    def _build_loss(self):
        assert self.class_frequencies is not None, "set class_frequencies before calling _build_loss"
        base_loss = AleatoricLossWrapper(
            AleatoricSegmentationLossFreqWeighted(
...
class nnUNetTrainerAleatoricFreqWeighted_AdultGlioma(nnUNetTrainerAleatoricFreqWeighted):
    class_frequencies = {
        1: 2051.210810810811,
...
class nnUNetTrainerAleatoricFreqWeighted_PediatricGlioma(nnUNetTrainerAleatoricFreqWeighted):
    class_frequencies = {
        1: 4231.806122448979,
